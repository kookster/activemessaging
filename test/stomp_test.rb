...
class StompTest < Test::Unit::TestCase
  ...
  def test_subscribe
    ...
    assert_equal({:destination=>@d}, @connection.stomp_connection.subscriptions[@d])
  end
  ...
  def test_send
    ...
    assert_equal "destination:#{@d}", @connection.stomp_connection.socket.sent_messages[1]
    assert_equal "content-length: #{@message.length}", @connection.stomp_connection.socket.sent_messages[2]
